  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <main className="flex-1 overflow-y-auto px-4 py-6 pb-32 space-y-6">
        ...
        <section>
          <h2 className="text-sm font-bold text-gray-900 uppercase tracking-wide mb-3 pl-1">Customer</h2>
          <ProfileCard 
        ...
        {/* Tool Details Card */}
        <section>
          <h2 className="text-sm font-bold text-gray-900 uppercase tracking-wide mb-3 pl-1">Rental Details</h2>
          <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 flex flex-col gap-4">
            ...
            <div className="flex items-start justify-between border-b border-gray-50 pb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-gray-900 text-base">{MOCK_RENTAL.tool.name}</h3>
                  <p className="text-sm font-medium text-gray-500 mt-0.5">Quantity: <span className="text-gray-900">{MOCK_RENTAL.tool.quantity}</span></p>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="flex flex-col gap-1">
                ...
                <span className="text-sm font-bold text-gray-900">{MOCK_RENTAL.rental.startDate}</span>
              </div>
              <div className="flex flex-col gap-1">
                <span className="flex items-center gap-1.5 text-xs font-semibold text-gray-500 uppercase tracking-wider">
                  <Clock className="w-3.5 h-3.5" /> End
                </span>
                <span className="text-sm font-bold text-gray-900">{MOCK_RENTAL.rental.endDate}</span>
              </div>
            </div>
        ...
        {/* Billing Calculation */}
        <section>
          <h2 className="text-sm font-bold text-gray-900 uppercase tracking-wide mb-3 pl-1">Cost Calculation</h2>
          <BillingSummary 
          ...
          />
          <p className="flex items-start gap-2 text-xs text-gray-500 font-medium mt-3 px-1">
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-4 pb-safe shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] z-50">
        <div className="max-w-md mx-auto flex gap-3">
          <button 
            onClick={() => router.back()}
            className="px-5 py-3.5 bg-white border border-gray-200 text-gray-700 font-bold rounded-xl text-center hover:bg-gray-50 transition-colors active:scale-95"
          >
            Cancel
          </button>
          <button 
            className="flex-1 px-5 py-3.5 bg-indigo-600 text-white font-bold rounded-xl text-center shadow-[0_2px_12px_-2px_rgba(79,70,229,0.4)] hover:bg-indigo-700 transition-all active:scale-[0.98]"
          >
            Mark as Returned
          </button>
        </div>
